...
--------------------------------------------------------------------------------
-- Process a form
FUNCTION process_form(l_fname STRING) RETURNS()
	DEFINE f_doc om.DomDocument
	DEFINE f_node om.DomNode
	DEFINE l_outname STRING
	DEFINE l_bakname STRING
	...
	LET f_node = f_node.loadXml(l_fname)
	...
	IF nl.getLength() = 0 AND ignore_genero_forms THEN
		DISPLAY "Genero Form already:", l_fname.trim(), " ignoring it."
		RETURN
	END IF
	...
	DISPLAY "Compiling ", l_outname.trim(), " ..."
	RUN "fglform " || l_outname.trim()
